...
async function iniciar() {
  ...
  if (state.token) {
    try {
      const eu = await api("/api/conta/eu");
      state.conta = eu;
      ...
      if (entrada) {
        state.grupo = { code: entrada.code, nome: entrada.nome, liga: entrada.liga };
        state.view = "bolao";
        render();
        entrarNoBolao();
        return;
...
window.criarGrupo = async function () {
  ...
  try {
    ...
    state.grupo = { code: r.code, nome: r.nome, liga: r.liga };
    state.erro = "";
    state.view = "bolao";
...
window.abrirGrupoLocal = function (code) {
  const entry = state.meusGrupos.find((g) => g.code === code);
  if (!entry) return;
  localStorage.setItem("bolao_ultimo_grupo", code);
  state.grupo = { code: entry.code, nome: entry.nome, liga: entry.liga };
  state.view = "bolao";
  state.abaBolao = "jogos";
  render();
  entrarNoBolao();
...
window.salvarPalpites = async function () {
  try {
    ...
    state.mensagemSalvo = "Palpites salvos!";
    render();
    const classi = await api(`/api/grupos/${state.grupo.code}/classificacao`);
    state.classificacao = classi.classificacao || [];
    render();
    setTimeout(() => { state.mensagemSalvo = ""; render(); }, 2200);
  } catch (e) {
    state.mensagemSalvo = "Não deu pra salvar agora.";
    render();
  }
};
...
function montarGraficoInline(historico, slug) {
  ...
  if (pontos.length < 2) {
    return `<div style="padding:22px 18px;text-align:center">
      <p class="f-mono" style="color:var(--paper-soft);font-size:13px;margin:0">
        Ainda não há histórico suficiente pra desenhar o gráfico — ele vai se formando conforme os jogos da rodada vão terminando.
      </p></div>`;
  }
  ...
  let resumo;
  if (diff > 0) resumo = `⬆ subiu ${diff} posição${diff > 1 ? "ões" : ""} desde o início da rodada`;
  else if (diff < 0) resumo = `⬇ caiu ${Math.abs(diff)} posição${Math.abs(diff) > 1 ? "ões" : ""} desde o início da rodada`;
  else resumo = "➡ manteve a posição desde o início da rodada";

  return `<div style="padding:16px 18px 20px;background:rgba(0,0,0,0.15);border-radius:10px;margin:4px 0 10px">
    <p class="f-mono" style="color:var(--paper-soft);font-size:12px;margin:0 0 10px">colocação ao longo dos jogos encerrados na rodada</p>
    ...
    <p class="f-mono" style="color:var(--paper-soft);font-size:11px;margin-top:2px;opacity:.8">eixo X: nº de jogos já encerrados na rodada · eixo Y: colocação no grupo (1º no topo)</p>
  </div>`;
}
...
function cartaoJogo(g) {
  ...
  let statusHtml;
  if (g.status === "live") statusHtml = `<span class="f-display" style="color:var(--live);font-weight:700;font-size:12px;display:flex;align-items:center;gap:5px"><span class="live-dot"></span>AO VIVO</span>`;
...
function abaJogos() {
  ...
  return `
    <div class="wrap" style="padding:18px 20px 30px">
      ...
      <p class="f-mono" style="font-size:11px;color:var(--ink-soft);margin:2px 0 14px">
      ...
      <div style="display:flex;align-items:center;gap:12px;margin-top:16px;flex-wrap:wrap">
        <button class="chip btn-amber f-display" style="text-transform:uppercase" onclick="salvarPalpites()">Salvar meus palpites</button>
        ${state.mensagemSalvo ? `<span class="f-mono" style="font-size:11px;color:var(--ink-soft)">${state.mensagemSalvo}</span>` : ""}
      </div>
    </div>`;
}
    ...
    <div class="scoreboard" style="min-height:100vh;padding:22px 20px">
      <div class="wrap" style="padding:0">
        ...
        ${linhas.length === 0 ? `<p class="f-mono" style="color:var(--paper-soft);font-size:13px">Ninguém entrou no grupo ainda além de você. Manda o código pra galera!</p>` : `
          ...
          <p class="f-mono" style="font-size:11px;color:var(--paper-soft);margin-top:6px;opacity:.75">toque no nome de alguém pra ver o gráfico de posição ao longo da rodada</p>
        `}
...
const FAQ = [
  ...
  { p: "Esqueci a senha do grupo, o que eu faço?", r: "Quem criou o grupo é quem sabe a senha — peça pra essa pessoa. Não existe recuperação automática." },
  ...
  return `
    <div class="wrap" style="padding:18px 20px 40px">
      ...
      <div class="card-panel" style="margin-bottom:16px">
        ...
        <p class="f-mono" style="font-size:12px;color:var(--ink-soft);margin:0 0 10px">Grupo atual: <b style="color:var(--ink)">${state.grupo.nome}</b> (${state.grupo.code})</p>
        <div style="display:flex;gap:8px;flex-wrap:wrap">
